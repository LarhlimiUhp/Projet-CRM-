...
```mermaid
xychart-beta
    title "Croissance du marché mondial du CRM 2025-2032"
    x-axis [2025, 2026, 2027, 2028, 2029, 2030, 2031, 2032]
    y-axis "Milliards USD" 0 --> 160
    bar [81.2, 88.3, 95.9, 104.3, 113.4, 123.2, 138.8, 156.3]
    line [81.2, 88.3, 95.9, 104.3, 113.4, 123.2, 138.8, 156.3]
```
...
```mermaid
pie showData
    title Répartition géographique du marché mondial CRM (2025)
    "Amérique du Nord" : 45
    "Europe" : 30
    "Asie-Pacifique" : 20
    "Reste du monde (MENA, Afrique...)" : 5
```
...
```mermaid
xychart-beta
    title "Taux d'adoption d'un CRM au Maroc par taille d'entreprise (%)"
    x-axis ["TPE (<10)", "PME (10-50)", "ETI (50-250)", "Grande entreprise (>250)"]
    y-axis "Taux d'adoption (%)" 0 --> 100
    bar [8, 20, 55, 75]
```
...
```mermaid
xychart-beta
    title "Niveau d'adoption CRM par secteur au Maroc (estimation %)"
    x-axis ["Télécoms", "Banque/Assurance", "Immobilier", "Retail", "Tourisme", "Services BtoB", "Industrie"]
    y-axis "Adoption (%)" 0 --> 100
    bar [85, 80, 55, 50, 50, 40, 30]
```
...
```mermaid
pie showData
    title Parts de marché CRM au Maroc (estimation 2026)
    "Microsoft Dynamics 365" : 22
    "Salesforce" : 20
    "Odoo CRM" : 15
    "Zoho CRM" : 12
...
```mermaid
quadrantChart
    title Positionnement des CRM sur le marché marocain
    x-axis "Faible coût" --> "Coût élevé"
    y-axis "Petite entreprise" --> "Grande entreprise"
    quadrant-1 "Premium grandes entreprises"
    quadrant-2 "Accessibles grandes entreprises"
    quadrant-3 "Accessibles PME"
    quadrant-4 "Premium PME"
    Salesforce: [0.90, 0.92]
    "Microsoft Dynamics 365": [0.75, 0.80]
    "SAP CX": [0.92, 0.95]
    "HubSpot Pro": [0.70, 0.55]
    "Odoo CRM": [0.40, 0.50]
    "Zoho CRM": [0.30, 0.35]
    "Vtiger CRM": [0.35, 0.40]
    "HubSpot Free": [0.05, 0.20]
    "Kommo": [0.25, 0.25]
    "Dolibarr": [0.15, 0.30]
```
...
```mermaid
flowchart TD
    A[Choix de la solution CRM] --> B{Où sont hébergées<br/>les données ?}
    B -->|Maroc| C[Déclaration préalable<br/>auprès de la CNDP]
    B -->|UE, Suisse, Canada| C
    B -->|Autres pays| D[Autorisation préalable<br/>de la CNDP requise]
    C --> E[Réponse CNDP<br/>sous 2 mois]
    D --> E
    E --> F[Affichage du récépissé<br/>dans les mentions légales]
    F --> G[Mise en place des<br/>droits des personnes :<br/>accès, rectification, opposition]
    G --> H[Sécurisation technique<br/>chiffrement, contrôle d'accès]
    H --> I[CRM conforme<br/>loi 09-08]
...
```mermaid
xychart-beta
    title "Impact du CRM sur les performances commerciales (%)"
    x-axis ["Hausse conversions", "Précision reporting", "Hausse CA", "Hausse fidélisation"]
    y-axis "Amélioration (%)" 0 --> 320
    bar [300, 42, 24, 47]
```
...
```mermaid
timeline
    title Stratégie Maroc Digital 2030 — Principaux jalons
    2024 : Lancement officiel (septembre)
         : 380 startups
         : 14 000 talents/an formés
    2026 : Phase 1 achevée
         : 11 Mds MAD investis
         : Décret TPME en vigueur
    2028 : Cloud souverain marocain
         : Couverture 5G 70%
         : Subventions MOWAKABA généralisées
    2030 : 3 000 startups visées
         : 100 000 talents formés/an
         : 240 000 emplois numériques
         : 7 Mds MAD levés
```
...
```mermaid
mindmap
  root((CRM 2026-2030<br/>au Maroc))
    IA générative
      Salesforce Einstein
      Microsoft Copilot
      HubSpot Breeze
...
```mermaid
flowchart TD
    Start[Entreprise marocaine<br/>souhaitant un CRM] --> Q1{Taille de<br/>l'entreprise ?}

    Q1 -->|TPE 1-10| Q2{Budget<br/>disponible ?}
    Q1 -->|PME 10-50| Q3{Besoin ERP<br/>intégré ?}
    Q1 -->|ETI 50-250| Q4{Écosystème<br/>existant ?}
    Q1 -->|GE 250+| Q5{Multinationale ?}

    Q2 -->|Quasi nul| R1[HubSpot Free<br/>ou Zoho Free]
    Q2 -->|Limité| R2[Kommo<br/>ou Zoho Standard]

    Q3 -->|Oui| R3[Odoo CRM<br/>+ ERP intégré]
    Q3 -->|Non| R4[Vtiger CRM<br/>ou Zoho Pro]

    Q4 -->|Microsoft 365| R5[Dynamics 365<br/>Sales Pro]
    Q4 -->|Hétérogène| R6[HubSpot<br/>Professional]

    Q5 -->|Oui| R7[Salesforce<br/>Enterprise]
    Q5 -->|Non| R8[Dynamics 365<br/>Enterprise]
    ...
    style R7 fill:#FFB6B6
    style R8 fill:#FFB6B6
```
...
```mermaid
flowchart LR
    A[1. Audit<br/>des processus] --> B[2. Cadrage<br/>des besoins]
    B --> C[3. Sélection<br/>de la solution]
    C --> D[4. Déclaration<br/>CNDP]
    D --> E[5. Paramétrage<br/>et personnalisation]
    E --> F[6. Migration<br/>des données]
    F --> G[7. Formation<br/>des utilisateurs]
    G --> H[8. Déploiement<br/>progressif]
    H --> I[9. Suivi des KPI<br/>et ajustements]
    I -->|Itération| E
